...
is a library, and also has a command line tool. The idea is that the generation
could be run on a server, where is has access to the two component modules
above but nothing apart from them. 
