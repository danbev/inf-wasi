...
others as this is a fast moving field. The target user of these are users
wanting to run llm inference on their local machine for reasons like privacy
(not sharing their data with an LLM inference service provider), or wanting to
avoid the cost of a LLM inference provider.
...
Yet another motivation is that the same .wasm component module can be used in
...
When choosing a model, or switching to a different model, to use one might want
to first test the model out and verify that the prompt to be used work with that
...
First we build the components:
```console
$ make build-engine build-config build-inference
$ make run-wasmedge-wasi-nn-example
$ make engine-component config-component inference-component
```
Note, that the config component is just a static configuration in this case.
Next we can compose the inference component with the engine and the config:
```console
$ make compose
composed component `target/composed.wasm`
```
And then we can run the composed component:
```console
$ make run-rust-bindings 
    Finished release [optimized] target(s) in 0.17s
     Running `target/release/rust-bindings --component-path ./target/composed.wasm --model-dir ./models`
Loaded component module.
model_dir: ./models
Running inference
LlamaCppBackend: model_path: "models/llama-2-7b-chat.Q5_K_M.gguf"
LlamaCppBackend: init_execution_context
...
```
I'm using a small model which is sometimes accurate but often not. And the
output is actually from the LlamaCppBackend which is a wasmtime backend that
uses the llama.cpp library and very much a work in progress. But I wanted to get
something working all the way through to verify that this idea is possible.
